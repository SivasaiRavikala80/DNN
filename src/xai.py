...
Methods
───────
1. Integrated Gradients  – primary technique 
...
from src.helpers import PALETTE, reverse_normalise

# ══════════════════════════════════════════════════════════════
# 1. Integrated Gradients
# ══════════════════════════════════════════════════════════════

def integrated_gradients(model, frames: torch.Tensor,
                          captions: torch.Tensor,
                          num_steps: int = 50) -> np.ndarray:
    """
    Compute integrated gradients attribution for each context frame.
    """

    # -------- FIX: Hybrid mode handling --------
    was_training = model.training

    # Set model to training mode (needed for GRU backward)
    model.train()
    ...
    attributions = torch.zeros(frames.shape[1])

    for step in range(num_steps):
        alpha = step / num_steps

        interpolated = (baseline + alpha * (frames - baseline)).detach()
        interpolated.requires_grad_(True)

        out = model(interpolated, captions)
        ...
        if interpolated.grad is not None:
            ...
            attributions += grad_mag.detach().cpu()

    # Restore original state
    if not was_training:
        model.eval()

    delta = (frames - baseline)[0].abs().sum(dim=(1, 2, 3)).cpu()
...
def run_all_xai(model, batch: dict, device: torch.device,
    ...
    # 1. Integrated Gradients
    attrs = integrated_gradients(model, frames, captions)
